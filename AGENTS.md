...
## Distilled Lessons (auto-generated by BSELA)

Rules distilled from real session errors. Confidence ≥ 0.85 auto-approved; others operator-reviewed.

- **CLI registration** (conf=0.90): After adding a CLI command or Typer group, run `bsela --help` and assert the command appears before committing. Add a test that captures help text if the project has a test suite.
- **Make targets** (conf=0.90): Before invoking any `make <target>`, confirm the target is defined in the Makefile. `make: *** No rule to make target` means the target is missing, not that the code is broken.
- **No identical tool calls** (conf=0.90): Never issue the same tool call (same name + arguments) twice in a row. Each invocation must add new context. If the second call would be identical, change strategy instead.
- **CLI argument validation** (conf=0.90): Validate CLI arguments at the handler entry point and surface a clear usage message before executing any logic. Don't let invalid input reach deep call stacks.
- **JSON parsing boundaries** (conf=0.78): Wrap all external JSON parsing (`json.loads`, `model_validate_json`) in try/except and surface a user-friendly error. Never let a parse error propagate as an unhandled exception.
- **Tool argument type coercion** (conf=0.92): Validate and coerce tool-invocation arguments to the expected schema types before the call — especially numeric parameters (`offset`, `limit`, `line`) that may arrive as strings. `InputValidationError` from a type mismatch means the call site is wrong, not the tool.
